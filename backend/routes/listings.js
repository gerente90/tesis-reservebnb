...
router.post('/', authMiddleware, async (req, res) => {
  const { nombre, descripcion, imagen, capacidad, camas, precio, servicios, airbnb_link } = req.body;
  try {
    console.log('Creating listing:', nombre, 'Airbnb link:', airbnb_link);
    const serviciosJson = JSON.stringify(servicios);
    const result = await pool.query(
      'INSERT INTO listings (nombre, descripcion, imagen, capacidad, camas, precio, servicios, airbnb_link) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
      [nombre, descripcion, imagen, capacidad, camas, precio, serviciosJson, airbnb_link]
    );
    res.json(result.rows[0]);
  } catch (err) {
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const serviciosJson = JSON.stringify(servicios);
    const result = await pool.query(
      'UPDATE listings SET nombre=$1, descripcion=$2, imagen=$3, capacidad=$4, camas=$5, precio=$6, servicios=$7, airbnb_link=$8 WHERE id=$9 RETURNING *',
      [nombre, descripcion, imagen, capacidad, camas, precio, serviciosJson, airbnb_link, id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Listing not found' });
    }
